...
module functions_module (
    ...
    reg [4-1:0] C_m_next, C_p_next; // next value for result dimensions
            ...
            CHECK_DIM: begin
                case (funSel)
                    `OP_TRANSPOSE: begin
                        ...
                        C_m_next = A_n; C_p_next = A_m;
                    end
                    `OP_ADD: begin
                        if (A_m != B_m || A_n != B_n) unableToOperate = 1'b1;
                        C_m_next = A_m; C_p_next = A_n;
                    end
                    `OP_SCALARMUL: begin
                        if (A_m == 0 || A_n == 0) unableToOperate = 1'b1;
                        C_m_next = A_m; C_p_next = A_n;
                    ...
                    `OP_MATMUL: begin
                        ...
                        C_m_next = A_m; C_p_next = B_n;
                    end
                    default: begin
                        unableToOperate = 1'b1;
                        C_m_next = 0; C_p_next = 0;
                    end
                endcase
    ...
    always @(posedge clk or posedge reset) begin
        if (reset) begin
            ...
            // Update dimension registers when transitioning to INIT
            if (state == CHECK_DIM && next_state == INIT) begin
                C_m <= C_m_next;
                C_p <= C_p_next;
            end
